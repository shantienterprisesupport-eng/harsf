...
import { agents, buildLocalAssistantReply, decideTask, planGoal, summarizeWorkflow } from './core/orchestrator';
...
const quickJobs = [
  ...
  'Naya app banao aur GitHub PR tak workflow chalao',
...
export default function App() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { id: 'welcome', author: 'ceo', text: 'Namaste! Main aapka HARSF Master AI Assistant hoon. Hindi, Hinglish, Odia ya English mein kaam bolo. App build, bug fix, automation, repo planning aur project coordination ke liye workflow banaunga. Payment, API key, destructive action, merge ya deploy se pehle aapka approval lunga.' },
  ]);
  ...
  const connectionLabel = gatewayReady
    ? `Master Assistant connected${activeProvider ? ` · ${activeProvider}${activeModel ? ` / ${activeModel}` : ''}` : ''}`
  ...
  return <main>
    ...
    <section className="hero">
      ...
      <h1>Kaam bolo. Assistant<br/><span>samjhega, plan karega, build workflow chalayega.</span></h1>
      <p>Voice ya text se command do. App build aur automation workflows specialist agents ko route hote hain; protected actions par final approval aapka rahega.</p>
    ...
    <section className="status-board" aria-label="Assistant status">
      ...
      <div className="status-card"><span>DOING</span><strong>{sending ? 'Master AI is thinking' : workflowStatus.doing}</strong></div>
    ...
    <div className="layout">
      <section className="chat panel">
        <div className="panel-title"><div><h2>Master Assistant Chat</h2><p><Languages size={14}/> Odia · Hindi · Hinglish · English</p></div><span className="live">{gatewayReady ? 'AI LIVE' : 'LOCAL PLAN'}</span></div>
        <div className="messages">{messages.map((m) => <div key={m.id} className={`message ${m.author}`}><span>{m.author === 'ceo' ? 'MASTER AI' : 'YOU'}</span>{m.text}</div>)}{sending && <div className="message ceo"><span>MASTER AI</span>Kaam samajh raha hoon…</div>}</div>
        <div className="composer">
          <textarea aria-label="Task command" value={input} disabled={sending} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); void submit(); } }} placeholder="Jaise: Mere liye booking app banao aur GitHub PR ready karo…"/>
